...
export const BooksContext = createContext<BooksContextType>(booksContextDefaultValue);
...
const MainPage: React.FC = () => {
    ...
    const [paginationBooksQuantity, setPaginationBooksQuantity] = useState<number>(10);
    const [offsetPagination, setOffsetPagination] = useState<number>(0);
    const [filteredBooks, setFilteredBooks] = useState<[] | BookType[] | undefined>([])

    useEffect(() => {
        dispatch(fetchBooks({searchQuery, newnessValue, offsetPagination, paginationBooksQuantity}));
    }, [searchQuery, newnessValue, paginationBooksQuantity]);

    const filteredBooksByCategory = (arr: BookType[], sortParam: string) => {
        return [...arr].filter(item => {
            return item?.volumeInfo?.categories?.find(category => {
                if (sortParam !== 'All') {
                ...
                {error
                    ...
                    : <>
                        ...
                        <div className={`container ${styles.mainPage}`}>
                            <h2 className={styles.mainPage__title}>Found {filteredBooks?.length} results</h2>
